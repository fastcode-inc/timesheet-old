...
import java.time.*;
import com.fastcode.timesheetapp1.addons.reporting.application.reportversion.IReportversionAppService;
import com.fastcode.timesheetapp1.addons.reporting.application.reportversion.IReportversionMapper;
...
import com.fastcode.timesheetapp1.addons.reporting.domain.report.QReportEntity;
...
public class ReportAppService implements IReportAppService {
	...
	public void delete(Long reportId,  Long userId) {
		...
		existing.removeReportversion(_reportversionRepository.findById(new ReportversionId(userId, reportId, "running")).get());
		existing.removeReportversion(_reportversionRepository.findById(new ReportversionId(userId, reportId, "published")).get());
	...
	public FindReportByIdOutput findById(Long reportId) {
		...
		ReportversionEntity reportversion = _reportversionRepository.findById(new ReportversionId(foundReport.getUsers().getId(), foundReport.getId(), "running")).orElse(null);
	...
	public List<FindReportByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		while (reportIterator.hasNext()) {
			...
			ReportversionEntity reportVersion =_reportversionRepository.findById(new ReportversionId(report.getUsers().getId(),report.getId(), "running")).orElse(null);
