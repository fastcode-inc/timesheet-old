...
public class TimeofftypeController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a timeofftype with a id=%s", id)));
	...
	public ResponseEntity<UpdateTimeofftypeOutput> update(@PathVariable String id, @RequestBody @Valid UpdateTimeofftypeInput timeofftype) {

	    FindTimeofftypeByIdOutput currentTimeofftype = _timeofftypeAppService.findById(Long.valueOf(id));
		Optional.ofNullable(currentTimeofftype).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Timeofftype with id=%s not found.", id)));
	...
	public ResponseEntity find(@RequestParam(value="search", required=false) String search, @RequestParam(value = "offset", required=false) String offset, @RequestParam(value = "limit", required=false) String limit, Sort sort) throws Exception {
		...
		if(sort == null || sort.isEmpty()) {
			sort = Sort.by(Sort.Direction.ASC, "typename");
		}
