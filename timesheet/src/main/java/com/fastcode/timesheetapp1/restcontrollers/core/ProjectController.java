...
import com.fastcode.timesheetapp1.application.core.customer.ICustomerAppService;
...
public class ProjectController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a project with a id=%s", id)));
	...
	public ResponseEntity<UpdateProjectOutput> update(@PathVariable String id, @RequestBody @Valid UpdateProjectInput project) {

	    FindProjectByIdOutput currentProject = _projectAppService.findById(Long.valueOf(id));
		Optional.ofNullable(currentProject).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Project with id=%s not found.", id)));
	...
	public ResponseEntity getTasks(@PathVariable String id, @RequestParam(value="search", required=false) String search, @RequestParam(value = "offset", required=false) String offset, @RequestParam(value = "limit", required=false) String limit, Sort sort)throws Exception {
   		if (offset == null) { offset = env.getProperty("fastCode.offset.default"); }
		if (limit == null) { limit = env.getProperty("fastCode.limit.default"); }
