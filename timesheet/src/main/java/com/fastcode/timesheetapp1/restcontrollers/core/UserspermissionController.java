...
import com.fastcode.timesheetapp1.application.core.authorization.permission.IPermissionAppService;
...
public class UserspermissionController {
	...
	public void delete(@PathVariable String id) {
		...
		Optional.ofNullable(userspermissionid).orElseThrow(() -> new EntityNotFoundException(String.format("Invalid id=%s", id)));

		FindUserspermissionByIdOutput output = _userspermissionAppService.findById(userspermissionid);
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a userspermission with a id=%s", id)));
	...
	public ResponseEntity<UpdateUserspermissionOutput> update(@PathVariable String id, @RequestBody @Valid UpdateUserspermissionInput userspermission) {
		...
		Optional.ofNullable(currentUserspermission).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Userspermission with id=%s not found.", id)));
