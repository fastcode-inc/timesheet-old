...
import com.fastcode.timesheetapp1.application.core.appconfiguration.IAppConfigurationAppService;
...
public class AppConfigurationController {
	...
	@NonNull protected final IAppConfigurationAppService _appConfigurationAppService;
	...
	public ResponseEntity<CreateAppConfigurationOutput> create(@RequestBody @Valid CreateAppConfigurationInput appConfiguration) {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a appConfiguration with a id=%s", id)));
	...
	public ResponseEntity<UpdateAppConfigurationOutput> update(@PathVariable String id, @RequestBody @Valid UpdateAppConfigurationInput appConfiguration) {

	    FindAppConfigurationByIdOutput currentAppConfiguration = _appConfigurationAppService.findById(Long.valueOf(id));
		Optional.ofNullable(currentAppConfiguration).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. AppConfiguration with id=%s not found.", id)));
