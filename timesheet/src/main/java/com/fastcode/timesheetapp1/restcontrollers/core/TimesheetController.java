...
public class TimesheetController {
	...
	@NonNull  protected final ITimesheetstatusAppService  _timesheetstatusAppService;
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a timesheet with a id=%s", id)));
	...
	public ResponseEntity<UpdateTimesheetOutput> update(@PathVariable String id, @RequestBody @Valid UpdateTimesheetInput timesheet) {

	    FindTimesheetByIdOutput currentTimesheet = _timesheetAppService.findById(Long.valueOf(id));
		Optional.ofNullable(currentTimesheet).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Timesheet with id=%s not found.", id)));
    ...
    @PreAuthorize("hasAnyAuthority('TIMESHEETENTITY_READ')")
	@RequestMapping(value = "/{id}/timesheetstatus", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
