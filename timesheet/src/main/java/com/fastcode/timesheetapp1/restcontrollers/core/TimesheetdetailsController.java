...
import com.fastcode.timesheetapp1.application.core.timeofftype.ITimeofftypeAppService;
...
public class TimesheetdetailsController {
	...
	@NonNull protected final ITimesheetdetailsAppService _timesheetdetailsAppService;

    @Qualifier("taskAppService")
	@NonNull  protected final ITaskAppService  _taskAppService;

    @Qualifier("timeofftypeAppService")
	@NonNull  protected final ITimeofftypeAppService  _timeofftypeAppService;
	...
	public ResponseEntity<CreateTimesheetdetailsOutput> create(@RequestBody @Valid CreateTimesheetdetailsInput timesheetdetails) {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a timesheetdetails with a id=%s", id)));
	...
	public ResponseEntity<UpdateTimesheetdetailsOutput> update(@PathVariable String id, @RequestBody @Valid UpdateTimesheetdetailsInput timesheetdetails) {

	    FindTimesheetdetailsByIdOutput currentTimesheetdetails = _timesheetdetailsAppService.findById(Long.valueOf(id));
		Optional.ofNullable(currentTimesheetdetails).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Timesheetdetails with id=%s not found.", id)));
    ...
    @PreAuthorize("hasAnyAuthority('TIMESHEETDETAILSENTITY_READ')")
	@RequestMapping(value = "/{id}/task", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
    ...
    @PreAuthorize("hasAnyAuthority('TIMESHEETDETAILSENTITY_READ')")
	@RequestMapping(value = "/{id}/timeofftype", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
    ...
    @PreAuthorize("hasAnyAuthority('TIMESHEETDETAILSENTITY_READ')")
	@RequestMapping(value = "/{id}/timesheet", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
