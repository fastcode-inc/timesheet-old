...
import com.fastcode.timesheetapp1.application.core.project.IProjectAppService;
...
import com.fastcode.timesheetapp1.application.core.usertask.dto.FindUsertaskByIdOutput;
import java.util.*;
import java.time.*;
...
public class TaskController {
	...
	@NonNull  protected final IProjectAppService  _projectAppService;
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a task with a id=%s", id)));
	...
	public ResponseEntity<UpdateTaskOutput> update(@PathVariable String id, @RequestBody @Valid UpdateTaskInput task) {
		...
		Optional.ofNullable(currentTask).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Task with id=%s not found.", id)));
    ...
    @PreAuthorize("hasAnyAuthority('TASKENTITY_READ')")
	@RequestMapping(value = "/{id}/project", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
    ...
    @PreAuthorize("hasAnyAuthority('TASKENTITY_READ')")
	@RequestMapping(value = "/{id}/timesheetdetails", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
	public ResponseEntity getTimesheetdetails(@PathVariable String id, @RequestParam(value="search", required=false) String search, @RequestParam(value = "offset", required=false) String offset, @RequestParam(value = "limit", required=false) String limit, Sort sort)throws Exception {
   		if (offset == null) { offset = env.getProperty("fastCode.offset.default"); }
		if (limit == null) { limit = env.getProperty("fastCode.limit.default"); }
