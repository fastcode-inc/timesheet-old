...
import com.fastcode.timesheetapp1.application.core.authorization.role.IRoleAppService;
...
public class UsersroleController {
	...
	public void delete(@PathVariable String id) {
		...
		Optional.ofNullable(usersroleid).orElseThrow(() -> new EntityNotFoundException(String.format("Invalid id=%s", id)));

		FindUsersroleByIdOutput output = _usersroleAppService.findById(usersroleid);
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a usersrole with a id=%s", id)));
	...
	public ResponseEntity<UpdateUsersroleOutput> update(@PathVariable String id, @RequestBody @Valid UpdateUsersroleInput usersrole) {
		...
		Optional.ofNullable(currentUsersrole).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Usersrole with id=%s not found.", id)));
