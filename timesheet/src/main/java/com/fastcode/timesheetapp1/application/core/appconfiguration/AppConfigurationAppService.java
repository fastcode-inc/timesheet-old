...
public class AppConfigurationAppService implements IAppConfigurationAppService {
	...
	@NonNull protected final IAppConfigurationRepository _appConfigurationRepository;

	@Qualifier("IAppConfigurationMapperImpl")
	@NonNull protected final IAppConfigurationMapper mapper;
	...
	public FindAppConfigurationByIdOutput findById(Long appConfigurationId) {
		...
		AppConfigurationEntity foundAppConfiguration = _appConfigurationRepository.findById(appConfigurationId).orElse(null);
		if (foundAppConfiguration == null)  
			return null; 
	...
	public List<FindAppConfigurationByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<AppConfigurationEntity> foundAppConfiguration = _appConfigurationRepository.findAll(search(search), pageable);
	...
	protected BooleanBuilder searchKeyValuePair(QAppConfigurationEntity appConfiguration, Map<String,SearchFields> map,Map<String,String> joinColumns) {
			 	...
			 	if(details.getValue().getOperator().equals("contains")) {
					...
					builder.and(appConfiguration.id.ne(Long.valueOf(details.getValue().getSearchValue())));
				  	...
				  	if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(appConfiguration.id.between(Long.valueOf(details.getValue().getStartingValue()), Long.valueOf(details.getValue().getEndingValue())));
