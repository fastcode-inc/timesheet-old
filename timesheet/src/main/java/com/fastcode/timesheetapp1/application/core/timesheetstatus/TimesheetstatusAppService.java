...
public class TimesheetstatusAppService implements ITimesheetstatusAppService {
	...
	@NonNull protected final ITimesheetstatusMapper mapper;
	...
	public FindTimesheetstatusByIdOutput findById(Long timesheetstatusId) {
		...
		TimesheetstatusEntity foundTimesheetstatus = _timesheetstatusRepository.findById(timesheetstatusId).orElse(null);
	...
	protected BooleanBuilder searchKeyValuePair(QTimesheetstatusEntity timesheetstatus, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			 	...
			 	if(details.getValue().getOperator().equals("contains")) {
					...
					builder.and(timesheetstatus.id.ne(Long.valueOf(details.getValue().getSearchValue())));
				  	...
				  	if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(timesheetstatus.id.between(Long.valueOf(details.getValue().getStartingValue()), Long.valueOf(details.getValue().getEndingValue())));
            ...
            if(details.getKey().replace("%20","").trim().equals("statusname")) {
				if(details.getValue().getOperator().equals("contains")) {
					builder.and(timesheetstatus.statusname.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
