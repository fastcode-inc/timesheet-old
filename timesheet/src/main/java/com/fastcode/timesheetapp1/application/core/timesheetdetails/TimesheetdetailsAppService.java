...
public class TimesheetdetailsAppService implements ITimesheetdetailsAppService {
	...
	@NonNull protected final ITimesheetdetailsRepository _timesheetdetailsRepository;

    @Qualifier("taskRepository")
	@NonNull protected final ITaskRepository _taskRepository;

    @Qualifier("timeofftypeRepository")
	@NonNull protected final ITimeofftypeRepository _timeofftypeRepository;
	...
	@NonNull protected final ITimesheetdetailsMapper mapper;
	...
	public FindTimesheetdetailsByIdOutput findById(Long timesheetdetailsId) {
		...
		TimesheetdetailsEntity foundTimesheetdetails = _timesheetdetailsRepository.findById(timesheetdetailsId).orElse(null);
	...
	public List<FindTimesheetdetailsByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<TimesheetdetailsEntity> foundTimesheetdetails = _timesheetdetailsRepository.findAll(search(search), pageable);
...
//		        list.get(i).replace("%20","").trim().equals("periodstartingdate") ||
	...
	protected BooleanBuilder searchKeyValuePair(QTimesheetdetailsEntity timesheetdetails, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		if(joinCol != null && joinCol.getKey().equals("timesheet")) {
			builder.and(timesheetdetails.timesheet.periodstartingdate.eq(SearchUtils.stringToLocalDate(joinCol.getValue())));
        }
        }
		return builder;
	}
