...
public class UserspermissionAppService implements IUserspermissionAppService {
	...
	public CreateUserspermissionOutput create(CreateUserspermissionInput input) {
        ...
        CreateUserspermissionOutput output = mapper.userspermissionEntityToCreateUserspermissionOutput(createdUserspermission);
	...
	public FindUserspermissionByIdOutput findById(UserspermissionId userspermissionId) {
		...
		UserspermissionEntity foundUserspermission = _userspermissionRepository.findById(userspermissionId).orElse(null);
...
//		        list.get(i).replace("%20","").trim().equals("lastname") ||
	...
	protected BooleanBuilder searchKeyValuePair(QUserspermissionEntity userspermission, Map<String,SearchFields> map,Map<String,String> joinColumns) {
			 ...
			 if(details.getKey().replace("%20","").trim().equals("usersId")) {
			 	if(details.getValue().getOperator().equals("contains")) {
					...
					builder.and(userspermission.usersId.ne(Long.valueOf(details.getValue().getSearchValue())));
				  	...
				  	if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(userspermission.usersId.between(Long.valueOf(details.getValue().getStartingValue()), Long.valueOf(details.getValue().getEndingValue())));
                   	} else if(StringUtils.isNumeric(details.getValue().getStartingValue())) {
