...
public class UsertaskControllerTest {
	...
	public static void cleanup() {
		...
		em.createNativeQuery("truncate table timesheet.usertask RESTART IDENTITY").executeUpdate();
		em.createNativeQuery("truncate table timesheet.task RESTART IDENTITY").executeUpdate();
	...
	public void setup() {
		...
		final UsertaskController usertaskController = new UsertaskController(usertaskAppService, taskAppService, usersAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(usertaskController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/usertask/taskid=" + usertask.getTaskid()+ ",userid=" + usertask.getUserid()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/usertask/taskid=999,userid=999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateUsertask_UsertaskDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/usertask").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteUsertask_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/usertask/taskid=999,userid=999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a usertask with a id=taskid=999,userid=999"));
	...
	//    Mockito.when(usertaskAppService.findById(new UsertaskId(entity.getTaskid(), entity.getUserid()))).thenReturn(output);
        Mockito.doReturn(output).when(usertaskAppService).findById(new UsertaskId(entity.getTaskid(), entity.getUserid()));
        
		mvc.perform(delete("/usertask/taskid="+ entity.getTaskid()+ ",userid="+ entity.getUserid()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateUsertask_UsertaskDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(usertask);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/usertask/taskid=999,userid=999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Usertask with id=taskid=999,userid=999 not found."));
	}    
	...
	public void UpdateUsertask_UsertaskExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/usertask/taskid=" + entity.getTaskid()+ ",userid=" + entity.getUserid()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/usertask?search=taskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/usertask?search=usertasktaskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property usertasktaskid not found!"));
	...
	public void GetTask_IdIsNotEmptyAndIdIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/usertask/taskid999/task")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid id=taskid999"));
	
	}    
	@Test
	public void GetTask_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
  
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/usertask/taskid=999,userid=999/task")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetTask_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/usertask/taskid=" + usertask.getTaskid()+ ",userid=" + usertask.getUserid()+ "/task")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetUsers_IdIsNotEmptyAndIdIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/usertask/taskid999/users")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid id=taskid999"));
	...
	public void GetUsers_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/usertask/taskid=999,userid=999/users")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetUsers_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/usertask/taskid=" + usertask.getTaskid()+ ",userid=" + usertask.getUserid()+ "/users")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
