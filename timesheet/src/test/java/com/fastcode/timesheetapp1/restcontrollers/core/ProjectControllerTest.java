...
public class ProjectControllerTest {
	...
	public void setup() {
		...
		final ProjectController projectController = new ProjectController(projectAppService, customerAppService, taskAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(projectController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/project/" + project.getId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/project/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateProject_ProjectDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/project").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteProject_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/project/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a project with a id=999"));
    ...
    //    Mockito.when(projectAppService.findById(entity.getId())).thenReturn(output);
		...
		mvc.perform(delete("/project/" + entity.getId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateProject_ProjectDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(project);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/project/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Project with id=999 not found."));
	}    
	...
	public void UpdateProject_ProjectExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/project/" + entity.getId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/project?search=id[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/project?search=projectid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property projectid not found!"));
	...
	public void GetCustomer_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/project/999/customer")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetCustomer_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/project/" + project.getId()+ "/customer")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetTasks_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/project/1/tasks?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetTasks_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(projectAppService.parseTasksJoinColumn("projectid")).thenReturn(joinCol);
		mvc.perform(get("/project/1/tasks?search=projectid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetTasks_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/project/1/tasks?search=projectid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
