...
public class TimeofftypeControllerTest {
	...
	public void setup() {
		...
		final TimeofftypeController timeofftypeController = new TimeofftypeController(timeofftypeAppService, timesheetdetailsAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(timeofftypeController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/timeofftype/" + timeofftype.getId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/timeofftype/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateTimeofftype_TimeofftypeDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/timeofftype").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteTimeofftype_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/timeofftype/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a timeofftype with a id=999"));
    ...
    //    Mockito.when(timeofftypeAppService.findById(entity.getId())).thenReturn(output);
		...
		mvc.perform(delete("/timeofftype/" + entity.getId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateTimeofftype_TimeofftypeDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(timeofftype);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/timeofftype/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Timeofftype with id=999 not found."));
	}    
	...
	public void UpdateTimeofftype_TimeofftypeExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/timeofftype/" + entity.getId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/timeofftype?search=id[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/timeofftype?search=timeofftypeid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property timeofftypeid not found!"));
	...
	public void GetTimesheetdetails_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/timeofftype/1/timesheetdetails?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetTimesheetdetails_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(timeofftypeAppService.parseTimesheetdetailsJoinColumn("timeofftypeid")).thenReturn(joinCol);
		mvc.perform(get("/timeofftype/1/timesheetdetails?search=timeofftypeid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetTimesheetdetails_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/timeofftype/1/timesheetdetails?search=timeofftypeid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
