...
public class TaskControllerTest {
	...
	public void setup() {
		...
		final TaskController taskController = new TaskController(taskAppService, projectAppService, timesheetdetailsAppService, usertaskAppService,
	logHelper,env);
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/task/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void DeleteTask_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/task/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a task with a id=999"));
	...
	public void UpdateTask_TaskDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(task);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/task/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Task with id=999 not found."));
	}    
	...
	public void GetProject_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/task/999/project")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetProject_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/task/" + task.getId()+ "/project")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetTimesheetdetails_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/task/1/timesheetdetails?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetTimesheetdetails_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(taskAppService.parseTimesheetdetailsJoinColumn("taskid")).thenReturn(joinCol);
		mvc.perform(get("/task/1/timesheetdetails?search=taskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetTimesheetdetails_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/task/1/timesheetdetails?search=taskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetUsertasks_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/task/1/usertasks?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetUsertasks_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(taskAppService.parseUsertasksJoinColumn("taskid")).thenReturn(joinCol);
		mvc.perform(get("/task/1/usertasks?search=taskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetUsertasks_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/task/1/usertasks?search=taskid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
