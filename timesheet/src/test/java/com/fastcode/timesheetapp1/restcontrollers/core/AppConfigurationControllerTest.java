...
public class AppConfigurationControllerTest {
	...
	public static void cleanup() {
		...
		em.createNativeQuery("truncate table timesheet.app_configuration RESTART IDENTITY").executeUpdate();
	...
	public void setup() {
		...
		final AppConfigurationController appConfigurationController = new AppConfigurationController(appConfigurationAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(appConfigurationController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/appConfiguration/" + appConfiguration.getId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/appConfiguration/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateAppConfiguration_AppConfigurationDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/appConfiguration").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteAppConfiguration_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/appConfiguration/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a appConfiguration with a id=999"));
    ...
    //    Mockito.when(appConfigurationAppService.findById(entity.getId())).thenReturn(output);
		...
		mvc.perform(delete("/appConfiguration/" + entity.getId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateAppConfiguration_AppConfigurationDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(appConfiguration);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/appConfiguration/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. AppConfiguration with id=999 not found."));
	}    
	...
	public void UpdateAppConfiguration_AppConfigurationExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/appConfiguration/" + entity.getId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/appConfiguration?search=id[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/appConfiguration?search=appConfigurationid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property appConfigurationid not found!"));
