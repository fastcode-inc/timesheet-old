...
import com.fastcode.timesheetapp1.domain.core.appconfiguration.*;
import com.fastcode.timesheetapp1.commons.search.*;
import com.fastcode.timesheetapp1.application.core.appconfiguration.dto.*;
import com.fastcode.timesheetapp1.domain.core.appconfiguration.QAppConfigurationEntity;
...
public class AppConfigurationAppServiceTest {
	...
	public void findAppConfigurationById_IdIsNotNullAndIdExists_ReturnAppConfiguration() {
		...
		Optional<AppConfigurationEntity> appConfigurationOptional = Optional.of((AppConfigurationEntity) appConfiguration);
		Mockito.when(_appConfigurationRepository.findById(anyLong())).thenReturn(appConfigurationOptional);
		
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.appConfigurationEntityToFindAppConfigurationByIdOutput(appConfiguration));
	}
    ...
    public void createAppConfiguration_AppConfigurationIsNotNullAndAppConfigurationDoesNotExist_StoreAppConfiguration() { 
        ...
        Mockito.when(_mapper.createAppConfigurationInputToAppConfigurationEntity(any(CreateAppConfigurationInput.class))).thenReturn(appConfigurationEntity); 
        Mockito.when(_appConfigurationRepository.save(any(AppConfigurationEntity.class))).thenReturn(appConfigurationEntity);

	   	Assertions.assertThat(_appService.create(appConfigurationInput)).isEqualTo(_mapper.appConfigurationEntityToCreateAppConfigurationOutput(appConfigurationEntity));
	...
	public void updateAppConfiguration_AppConfigurationIdIsNotNullAndIdExists_ReturnUpdatedAppConfiguration() {
		...
		Optional<AppConfigurationEntity> appConfigurationOptional = Optional.of((AppConfigurationEntity) appConfigurationEntity);
		Mockito.when(_appConfigurationRepository.findById(anyLong())).thenReturn(appConfigurationOptional);
	 		
		Mockito.when(_mapper.updateAppConfigurationInputToAppConfigurationEntity(any(UpdateAppConfigurationInput.class))).thenReturn(appConfigurationEntity);
		Mockito.when(_appConfigurationRepository.save(any(AppConfigurationEntity.class))).thenReturn(appConfigurationEntity);
		Assertions.assertThat(_appService.update(ID,appConfiguration)).isEqualTo(_mapper.appConfigurationEntityToUpdateAppConfigurationOutput(appConfigurationEntity));
	}
	...
	public void find_ListIsEmpty_ReturnList() throws Exception {
		...
		Mockito.when(_appConfigurationRepository.findAll(any(Predicate.class),any(Pageable.class))).thenReturn(foundPage);
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QAppConfigurationEntity.class), any(HashMap.class), any(HashMap.class));
