...
import com.fastcode.timesheetapp1.domain.core.timeofftype.*;
import com.fastcode.timesheetapp1.commons.search.*;
import com.fastcode.timesheetapp1.application.core.timeofftype.dto.*;
import com.fastcode.timesheetapp1.domain.core.timeofftype.QTimeofftypeEntity;
...
public class TimeofftypeAppServiceTest {
	...
	public void findTimeofftypeById_IdIsNotNullAndIdExists_ReturnTimeofftype() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.timeofftypeEntityToFindTimeofftypeByIdOutput(timeofftype));
	}
    ...
    public void createTimeofftype_TimeofftypeIsNotNullAndTimeofftypeDoesNotExist_StoreTimeofftype() { 
        ...
        Mockito.when(_mapper.createTimeofftypeInputToTimeofftypeEntity(any(CreateTimeofftypeInput.class))).thenReturn(timeofftypeEntity); 
        Mockito.when(_timeofftypeRepository.save(any(TimeofftypeEntity.class))).thenReturn(timeofftypeEntity);

	   	Assertions.assertThat(_appService.create(timeofftypeInput)).isEqualTo(_mapper.timeofftypeEntityToCreateTimeofftypeOutput(timeofftypeEntity));
	...
	public void updateTimeofftype_TimeofftypeIdIsNotNullAndIdExists_ReturnUpdatedTimeofftype() {
		...
		Mockito.when(_mapper.updateTimeofftypeInputToTimeofftypeEntity(any(UpdateTimeofftypeInput.class))).thenReturn(timeofftypeEntity);
		Mockito.when(_timeofftypeRepository.save(any(TimeofftypeEntity.class))).thenReturn(timeofftypeEntity);
		Assertions.assertThat(_appService.update(ID,timeofftype)).isEqualTo(_mapper.timeofftypeEntityToUpdateTimeofftypeOutput(timeofftypeEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QTimeofftypeEntity.class), any(HashMap.class), any(HashMap.class));
