...
import com.fastcode.timesheetapp1.domain.core.usertask.*;
import com.fastcode.timesheetapp1.commons.search.*;
import com.fastcode.timesheetapp1.application.core.usertask.dto.*;
import com.fastcode.timesheetapp1.domain.core.usertask.QUsertaskEntity;
...
public class UsertaskAppServiceTest {
	...
	public void findUsertaskById_IdIsNotNullAndIdExists_ReturnUsertask() {
	    ...
	    Assertions.assertThat(_appService.findById(usertaskId)).isEqualTo(_mapper.usertaskEntityToFindUsertaskByIdOutput(usertask));
	}
    ...
    public void createUsertask_UsertaskIsNotNullAndUsertaskDoesNotExist_StoreUsertask() { 
        ...
        Mockito.when(_mapper.createUsertaskInputToUsertaskEntity(any(CreateUsertaskInput.class))).thenReturn(usertaskEntity); 
        Mockito.when(_usertaskRepository.save(any(UsertaskEntity.class))).thenReturn(usertaskEntity);

	   	Assertions.assertThat(_appService.create(usertaskInput)).isEqualTo(_mapper.usertaskEntityToCreateUsertaskOutput(usertaskEntity));
	...
	public void createUsertask_UsertaskIsNotNullAndUsertaskDoesNotExistAndChildIsNullAndChildIsNotMandatory_StoreUsertask() {
		...
		Mockito.when(_mapper.createUsertaskInputToUsertaskEntity(any(CreateUsertaskInput.class))).thenReturn(usertaskEntity);
		Mockito.when(_usertaskRepository.save(any(UsertaskEntity.class))).thenReturn(usertaskEntity);
	    Assertions.assertThat(_appService.create(usertask)).isEqualTo(_mapper.usertaskEntityToCreateUsertaskOutput(usertaskEntity)); 
	}
	...
	public void updateUsertask_UsertaskIsNotNullAndUsertaskDoesNotExistAndChildIsNullAndChildIsNotMandatory_ReturnUpdatedUsertask() {
		...
		Mockito.when(_mapper.updateUsertaskInputToUsertaskEntity(any(UpdateUsertaskInput.class))).thenReturn(usertaskEntity);
		Mockito.when(_usertaskRepository.save(any(UsertaskEntity.class))).thenReturn(usertaskEntity);
		Assertions.assertThat(_appService.update(usertaskId,usertask)).isEqualTo(_mapper.usertaskEntityToUpdateUsertaskOutput(usertaskEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QUsertaskEntity.class), any(HashMap.class), any(HashMap.class));
	...
	public void GetTask_IfUsertaskIdAndTaskIdIsNotNullAndUsertaskExists_ReturnTask() {
		...
		Assertions.assertThat(_appService.getTask(usertaskId)).isEqualTo(_mapper.taskEntityToGetTaskOutput(taskEntity, usertask));
	}
	...
	public void GetUsers_IfUsertaskIdAndUsersIdIsNotNullAndUsertaskExists_ReturnUsers() {
		...
		Assertions.assertThat(_appService.getUsers(usertaskId)).isEqualTo(_mapper.usersEntityToGetUsersOutput(usersEntity, usertask));
	}
