...
import com.fastcode.timesheetapp1.domain.core.timesheet.*;
import com.fastcode.timesheetapp1.commons.search.*;
import com.fastcode.timesheetapp1.application.core.timesheet.dto.*;
import com.fastcode.timesheetapp1.domain.core.timesheet.QTimesheetEntity;
...
public class TimesheetAppServiceTest {
	...
	public void findTimesheetById_IdIsNotNullAndIdExists_ReturnTimesheet() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.timesheetEntityToFindTimesheetByIdOutput(timesheet));
	}
    ...
    public void createTimesheet_TimesheetIsNotNullAndTimesheetDoesNotExist_StoreTimesheet() { 
        ...
        Mockito.when(_mapper.createTimesheetInputToTimesheetEntity(any(CreateTimesheetInput.class))).thenReturn(timesheetEntity); 
        Mockito.when(_timesheetRepository.save(any(TimesheetEntity.class))).thenReturn(timesheetEntity);

	   	Assertions.assertThat(_appService.create(timesheetInput)).isEqualTo(_mapper.timesheetEntityToCreateTimesheetOutput(timesheetEntity));
	...
	public void updateTimesheet_TimesheetIsNotNullAndTimesheetDoesNotExistAndChildIsNullAndChildIsMandatory_ReturnNull() {
		...
		Mockito.when(_mapper.updateTimesheetInputToTimesheetEntity(any(UpdateTimesheetInput.class))).thenReturn(timesheetEntity); 
	...
	public void updateTimesheet_TimesheetIdIsNotNullAndIdExists_ReturnUpdatedTimesheet() {
		...
		Mockito.when(_mapper.updateTimesheetInputToTimesheetEntity(any(UpdateTimesheetInput.class))).thenReturn(timesheetEntity);
		Mockito.when(_timesheetRepository.save(any(TimesheetEntity.class))).thenReturn(timesheetEntity);
		Assertions.assertThat(_appService.update(ID,timesheet)).isEqualTo(_mapper.timesheetEntityToUpdateTimesheetOutput(timesheetEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QTimesheetEntity.class), any(HashMap.class), any(HashMap.class));
	...
	public void GetTimesheetstatus_IfTimesheetIdAndTimesheetstatusIdIsNotNullAndTimesheetExists_ReturnTimesheetstatus() {
		...
		Assertions.assertThat(_appService.getTimesheetstatus(ID)).isEqualTo(_mapper.timesheetstatusEntityToGetTimesheetstatusOutput(timesheetstatusEntity, timesheet));
	}
	...
	public void GetUsers_IfTimesheetIdAndUsersIdIsNotNullAndTimesheetExists_ReturnUsers() {
		...
		Assertions.assertThat(_appService.getUsers(ID)).isEqualTo(_mapper.usersEntityToGetUsersOutput(usersEntity, timesheet));
	}
