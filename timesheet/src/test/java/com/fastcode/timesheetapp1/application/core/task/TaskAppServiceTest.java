...
import com.fastcode.timesheetapp1.domain.core.task.*;
...
public class TaskAppServiceTest {
    ...
    public void createTask_TaskIsNotNullAndTaskDoesNotExist_StoreTask() { 
        ...
        Mockito.when(_taskRepository.save(any(TaskEntity.class))).thenReturn(taskEntity);
	   	...
	   	Assertions.assertThat(_appService.create(taskInput)).isEqualTo(_mapper.taskEntityToCreateTaskOutput(taskEntity));
	...
	public void GetProject_IfTaskIdAndProjectIdIsNotNullAndTaskExists_ReturnProject() {
		...
		Assertions.assertThat(_appService.getProject(ID)).isEqualTo(_mapper.projectEntityToGetProjectOutput(projectEntity, task));
	}
