...
import com.fastcode.timesheetapp1.domain.core.project.*;
import com.fastcode.timesheetapp1.commons.search.*;
import com.fastcode.timesheetapp1.application.core.project.dto.*;
import com.fastcode.timesheetapp1.domain.core.project.QProjectEntity;
...
public class ProjectAppServiceTest {
	...
	public void findProjectById_IdIsNotNullAndIdExists_ReturnProject() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.projectEntityToFindProjectByIdOutput(project));
	}
    ...
    public void createProject_ProjectIsNotNullAndProjectDoesNotExist_StoreProject() { 
        ...
        Mockito.when(_mapper.createProjectInputToProjectEntity(any(CreateProjectInput.class))).thenReturn(projectEntity); 
        Mockito.when(_projectRepository.save(any(ProjectEntity.class))).thenReturn(projectEntity);

	   	Assertions.assertThat(_appService.create(projectInput)).isEqualTo(_mapper.projectEntityToCreateProjectOutput(projectEntity));
	...
	public void updateProject_ProjectIsNotNullAndProjectDoesNotExistAndChildIsNullAndChildIsMandatory_ReturnNull() {
		...
		Mockito.when(_mapper.updateProjectInputToProjectEntity(any(UpdateProjectInput.class))).thenReturn(projectEntity); 
	...
	public void updateProject_ProjectIdIsNotNullAndIdExists_ReturnUpdatedProject() {
		...
		Mockito.when(_mapper.updateProjectInputToProjectEntity(any(UpdateProjectInput.class))).thenReturn(projectEntity);
		Mockito.when(_projectRepository.save(any(ProjectEntity.class))).thenReturn(projectEntity);
		Assertions.assertThat(_appService.update(ID,project)).isEqualTo(_mapper.projectEntityToUpdateProjectOutput(projectEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QProjectEntity.class), any(HashMap.class), any(HashMap.class));
	...
	public void GetCustomer_IfProjectIdAndCustomerIdIsNotNullAndProjectExists_ReturnCustomer() {
		...
		Assertions.assertThat(_appService.getCustomer(ID)).isEqualTo(_mapper.customerEntityToGetCustomerOutput(customerEntity, project));
	}
